...
self.addEventListener('fetch', event => {
    ...
    console.log('from sw.js:', location.origin + url.origin);
...
async function networkFirst(req) {
    const cache = await caches.open(CACHE_NAME+'news-dynamic');
    ...
    try {
        ...
        return await cache.match(req) || await caches.match('/news/fallback.json');
    }
}